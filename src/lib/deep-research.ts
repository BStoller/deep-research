...
import { trimPrompt, mini4oModel, o1MiniModel } from "../ai/providers";
...
async function generateSerpQueries({
  ...
  console.log(
    "[deep-research.ts][generateSerpQueries] Created",
    res.object.queries.length,
    "queries:",
    res.object.queries
  );
...
async function processSerpResult({
  ...
  console.log(
    `[deep-research.ts][processSerpResult] Ran query: "${query}", found ${trimmedContents.length} contents`
  );
  ...
  console.log(
    "[deep-research.ts][processSerpResult] Prompt sent to LLM:",
    promptStr
  );
  ...
  const res = await generateObject({
    ...
    schema: z.object({
      ...
      followUpQuestions: z
        .array(z.string())
        .describe(
          `List of follow-up questions, max of ${numFollowUpQuestions}`
        ),
    }),
  });

  console.log(
    "[deep-research.ts][processSerpResult] LLM response:",
    res.object
  );
  console.log(
    `[deep-research.ts][processSerpResult] Created ${res.object.learnings.length} learnings:`,
      ...
      limit(async () => {
        try {
          ...
          if (newDepth > 0) {
            ...
            const nextQuery = `Previous research goal: ${
          ...
          if (e.message && e.message.includes("Timeout")) {
            console.error(
              `[deep-research.ts] Timeout error running query: "${serpQuery.query}":`,
              e
            );
          } else {
            console.error(
              `[deep-research.ts] Error running query: "${serpQuery.query}":`,
              e
            );
          }
